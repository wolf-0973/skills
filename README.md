...
1. `/grill`：能讲清但有关键取舍 → 一轮一问；维护 `CONTEXT.md`。大而模糊、单会话装不下 → 改走 `/scout`。
...
6. `/code-review`：并行 lens fan-out（machine / critical / quality / spec），critical 逐条证伪复核后，合并成按 critical → warning → nit 排序的单份报告。Spec lens 默认对照当前 slice / AC（用户指定才审整份 spec）。无定点时审工作区未提交 diff。

上下文建议：`/grill` → `/to-spec` → `/breakdown` 尽量同窗；每个 `/implement` 新开窗口只做一条 slice。
...
| Skill | 触发方式 | 一句话职责 |
| --- | --- | --- |
| [grill](./skills/grill/SKILL.md) | `/grill`、追问、对齐 | 一次一个决策地追问，沉淀术语；闭合决策交 `/to-spec` 吸收。 |
...
长期知识按职责归档：**`CONTEXT.md` 只收术语**；现有 **`AGENTS.md`** 可承接经用户确认的项目级协作规则。其余产物都是临时脚手架，**agent 不自动删，想清理时由用户删**。`<slug>` = 该 Feature / 主题的短横线命名（如 `export-data`）。
