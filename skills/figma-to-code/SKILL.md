...
description: >-
  ...
  UI code only after Figma design data is fetched successfully.
license: MIT
metadata:
  author: wolf
  version: "1.0.0"
compatibility: Requires a Figma MCP that can read design nodes, screenshot, and export assets (official Figma MCP preferred; Framelink works), plus browser screenshot capability (e.g. Chrome DevTools MCP).
---
...
- Capability gate: 确认 Figma MCP（须能读设计节点数据；官方 Figma MCP 优先，Framelink 亦可）、浏览器截图能力、仓库读写能力；配置见 [references/mcp-setup.md](references/mcp-setup.md)。
- Data gate: 设计数据获取成功前只做探测与规划；失败即输出 `Figma MCP: <错误类型>` 并停止。
...
3. **Fetch Figma data.** 从 URL 取 fileKey 与 node-id，调用所连 Figma MCP 的设计数据工具（官方 `get_design_context` / `get_metadata`，或 Framelink `get_figma_data`）。Completion: 节点树可用，`visible: false` 节点进入 Assumptions。
4. **Plan structure and anchors.** 按 [references/pipeline-details.md](references/pipeline-details.md) 完成 region checklist、关键区域尺寸锚点、样式映射与资产计划。Completion: 代码前的结构和尺寸证据齐全。
...
- `scripts/setup-check.mjs [figmaToken]` — 环境自检（token 仅 Framelink 分支需要）
